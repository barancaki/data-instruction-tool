...
if "mcexpocomfort.it" in url:
    st.info("Target: Mostra Convegno Expocomfort")
    scroll_count = st.number_input("How many times to scroll (load more)?", min_value=1, value=5)

    if st.button("Scan"):
        with st.spinner("Scanning..."):
            st.session_state['function_name'] = 'mostra_convegno'
            scrape_mostra_convegno(scroll_count)
        st.success("Scan complete!")
        st.balloons()

if "exhibitors.lopec.com" in url:
    st.info("LOPEC 2026 Scraper Selected")
    iterations = st.number_input("How many 'Next' clicks?", min_value=1, value=1)
    if st.button("Start Scan"):
        st.session_state['function_name'] = "lopec_2026"
        with st.spinner("Scraping LOPEC 2026..."):
            scrape_lopec(iterations)
        st.success("The scan is complete!")
        st.balloons()

if "wammorocco.com" in url:
    st.info("Target: WAM Morocco 2026 Exhibitor List")
    scroll_count = st.number_input("How many times to scroll (load more)?", min_value=1, value=5)

    if st.button("Start Scan"):
        with st.spinner("Scanning WAM Morocco..."):
            st.session_state['function_name'] = 'wam_morocco'
            scrape_wam_morocco(scroll_count)
        st.success("Scan complete!")
        st.balloons()
...
if "chillventa.de" in url:
    ...
    iterations = st.number_input("How many times to click 'Show more'?", min_value=1, value=5)
    
    if st.button("Start Scan"):
        with st.spinner("Scanning Chillventa..."):
            st.session_state['function_name'] = 'chillventa'
            scrape_chillventa(iterations)
        st.success("Scan complete!")
        st.balloons()

if "embedded-world.de/en/exhibitors-products/find-exhibitors" in url:
    st.info("Target: Embedded World Exhibitor List")
    page_count = st.number_input("How many Algolia pages do you want to scrape? (100 companies per page)", min_value=1, value=3, step=1, key="embedded_world_page_count")

    if st.button("Start Scan", key="embedded_world_start_scan"):
        with st.spinner("Scanning Embedded World exhibitors..."):
            st.session_state['function_name'] = 'embedded_world'
            scrape_embedded_world(int(page_count))
        st.success("Scan complete!")
        st.balloons()


if "perpa.com/perpa-firmalar" in url:
    st.info("Target: Perpa Firmalar")
    page_count = st.number_input("How many pages do you want to scrape?", min_value=1, value=3, step=1, key="perpa_page_count")

    if st.button("Start Scan", key="perpa_start_scan"):
        with st.spinner("Scanning Perpa company pages..."):
            st.session_state['function_name'] = 'perpa_firmalar'
            scrape_perpa_firmalar(int(page_count))
        st.success("Scan complete!")
        st.balloons()

if "global-industrie.com/en/exhibitors-list" in url:
    st.info("Target: Global Industrie Exhibitors List")
    load_more_count = st.number_input(
        "How many times should 'Load more' be clicked? (Approx. +50 records per click)",
        min_value=0,
        value=5,
        step=1,
        key="global_industrie_load_more_count",
    )

    if st.button("Start Scan", key="global_industrie_start_scan"):
        with st.spinner("Scanning Global Industrie exhibitors..."):
            st.session_state['function_name'] = 'global_industrie'
            scrape_global_industrie_exhibitors(int(load_more_count))
        st.success("Scan complete!")
        st.balloons()
