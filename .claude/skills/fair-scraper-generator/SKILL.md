...
## Pre-requisites
Before generating code, you must ask the user for:
...
- **Use the Template**: Follow the structure in `.claude/skills/fair-scraper-generator/references/scraper_template.py`.
...
- **UI Block**: Append a new `if "keyword" in url:` block following the pattern in `.claude/skills/fair-scraper-generator/references/ui_template.py`.
